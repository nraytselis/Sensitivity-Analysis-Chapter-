...
sim1select = data.frame(sim1) %>% select(c(1:4,66:68))

sim1select = left_join(sim1select,sim1Esrow,by="time")
...
ggplot(data=sim1summarizedlong, aes(x=time, y=Counts, group=PARAM, colour=PARAM)) + 
         geom_line(size = 1.4) + labs(y = "Counts", x = "time") + scale_y_log10()
